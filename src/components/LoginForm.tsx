...
const LoginForm: React.FC<LoginFormProps> = ({ onSuccess, onFailure, ...props }) => {
  const { config, openidConfiguration, token, setToken, setIdentity } = React.useContext(identityContext) as IdentityContextType;
  ...
  const authzEndpointURL: string = React.useMemo(() => {
    if (openidConfiguration === undefined || state === undefined) return
